function [ws, sigma, wp, G, ll] = GainNoise_fitter(s,m,c,varargin)
%% GainNoise_fitter
%
...
%% Defaults
sm_default = linspace(0.01,80,100);
Gprime_default = linspace(0.01,2,100);
...
addParameter(Parser,'params0',[0.1,0.1,0.1,1])
addParameter(Parser,'lb',[0, 0, 0, 0])
...
[params, ll, EXITFLAG,OUTPUT] = fmincon(@(p)(minimizer(p,s,m,c,f,sm,Gprime)),...
...
function out = minimizer(p,s,m,c,f,sm,Gprime)
    gains = p(4:end);
    g = gains(c);
    out = -LogLikelihood_data_take_gainNoise(s,m,g,p(1),p(3),p(2),...
        'f',f,'sm',sm,'Gprime',Gprime);
