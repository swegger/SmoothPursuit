...
integrand = integrand.*p_m_take_Gprime_sm(M,SM,GP,0.01,ws,b);
...
function out = p_m_take_Gprime_sm(m,sm,Gprime,wp,ws,b)
    %out = double(m == f(sm,Gprime));
    out = 1./sqrt(2*pi*wp^2*BLSlocal(sm,Gprime,ws).^2) .* exp( -(m - BLSlocal(sm,Gprime,ws)-b).^2./(2*wp^2*BLSlocal(sm,Gprime,ws).^2) );
    
function out = p_sm_take_s(sm,s,ws)
    out = 1./sqrt(2*pi*ws.^2*s.^2) .* exp( -(sm-s).^2./(2*ws^2*s.^2) );
    
function out = BLSlocal(sm,Gprime,ws)
    method.type = 'quad';
    method.dx = 1;
