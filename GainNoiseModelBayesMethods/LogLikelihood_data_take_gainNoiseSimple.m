function ll = LogLikelihood_data_take_gainNoiseSimple(s,m,G,ws,sigma,varargin)
...
parse(Parser,s,m,G,ws,sigma,varargin{:})
...
pS = p_sm_take_s(SM,S,ws);
pG = p_Gprime_take_G(GP,Gtemp,sigma);
pM = p_m_take_Gprime_sm(M,SM,GP,f,0.01,ws);
...
function out = p_m_take_Gprime_sm(m,sm,Gprime,f,wp,ws)
    %out = double(m == f(sm,Gprime));
    out = 1./sqrt(2*pi*wp^2*f(sm,Gprime,ws).^2) .* exp( -(m - f(sm,Gprime,ws)).^2./(2*wp^2*f(sm,Gprime,ws).^2) );
    
function out = p_sm_take_s(sm,s,ws)
    out = 1./sqrt(2*pi*ws.^2*s.^2) .* exp( -(sm-s).^2./(2*ws^2*s.^2) );
    
function out = simpleGainNoise(sm,Gprime,ws)
    out = Gprime.*sm;
%     method.type = 'quad';
%     method.dx = 2;
%     out = Gprime.*reshape(ScalarBayesEstimators(...
%         reshape(sm,[numel(sm),1]),ws,0.1,20,'method',method),...
%         size(Gprime));
